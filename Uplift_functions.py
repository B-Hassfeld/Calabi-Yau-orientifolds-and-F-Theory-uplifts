...
def glsm_Weierstrass(glsm,line_bundle_weights):
    ...
    **Description:**
    ...
    Constructs a GLSM charge matrix for a Weierstrass uplift from a base GLSM and line-bundle weights. This helper is deprecated.

    **Arguments:**

    - `glsm (numpy.ndarray)`: The base GLSM charge matrix.
    - `line_bundle_weights (array-like)`: The divisor weights defining the Weierstrass twist.

    **Returns:**

    - `numpy.ndarray`: The extended GLSM charge matrix.
...
def is_trilayer(p,get_index=False):
    ...
    **Description:**
    ...
    Checks whether a polytope has the trilayer structure by testing whether half the anticanonical GLSM charge appears among the vertex charge vectors. This helper is deprecated.

    **Arguments:**

    - `p (Polytope)`: The polytope to test.
    - `get_index (bool)`: Whether to also return the index of the distinguished vertex. Defaults to `False`.

    **Returns:**

    - `bool` or `tuple`: Whether the polytope is trilayer, optionally together with the distinguished vertex index.
...
def rotate_points(pts,pts6):
    ...
    **Description:**
    ...
    Applies an integral block transformation aligning the base directions of an uplift point configuration with a chosen point configuration. This helper is deprecated.

    **Arguments:**

    - `pts (numpy.ndarray)`: Reference base point configuration.
    - `pts6 (numpy.ndarray)`: Uplift point configuration.

    **Returns:**

    - `numpy.ndarray`: The rotated uplift point configuration.
...
def glsm_uplift_toric_base(glsm):
    ...
    **Description:**
    ...
    Constructs a GLSM charge matrix for a toric Weierstrass uplift of a base GLSM. This helper is deprecated.
    ...
    **Returns:**
    ...
    - `numpy.ndarray`: The uplift GLSM charge matrix.
...
def get_fixed_loci(TF,q):
    ...
    **Description:**
    ...
    Searches for the lowest-dimensional fixed locus of a toric `Z_2` action. This helper is deprecated.

    **Arguments:**

    - `TF (Fan)`: The toric fan.
    - `q (array-like)`: Integer vector defining the `Z_2` action.

    **Returns:**

    - `int` or `None`: The cone size of the first fixed locus found, or `None` if none is found.
...
def normal_fan_OLD(polytopes,inequalities=None,maximal_refinement=False,triangulate_refinement=False,return_unrefined_fan=False):
    ...
    Deprecated normal-fan construction.

    **Description:**

    Older implementation of `normal_fan`. Constructs the normal fan of a polytope or Minkowski sum and optionally constructs a maximal refinement subject to inequalities.
